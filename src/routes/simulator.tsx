...
import type { DriverInput, Snapshot } from "@/sim/types"
...
export function SimulatorPage() {
  ...
  return <section className="space-y-4">
    ...
    {snapshot.ended && <div className="rounded-xl border border-primary bg-card p-6 text-center"><h2 className="text-2xl font-semibold">Match complete</h2><p className="mt-2 text-muted-foreground">Red {snapshot.redScore} — Blue {snapshot.blueScore}</p>{snapshot.finalBreakdown && <p className="mt-2 text-sm text-muted-foreground">Red: {snapshot.finalBreakdown.red.cones} cone + {snapshot.finalBreakdown.red.runs} runs + {snapshot.finalBreakdown.red.tictactoes} tic-tac-toe + {snapshot.finalBreakdown.red.auto} auto. Blue: {snapshot.finalBreakdown.blue.cones} cone + {snapshot.finalBreakdown.blue.runs} runs + {snapshot.finalBreakdown.blue.tictactoes} tic-tac-toe + {snapshot.finalBreakdown.blue.auto} auto.</p>}<div className="mt-4 flex justify-center gap-2"><Button onClick={() => { setSnapshot(null); setSaved(false); setMatchNumber((current) => current + 1) }}>Run again</Button><Button onClick={() => navigate("/")} variant="outline">Back to setup</Button></div></div>}
  </section>
}
